...
import { getTokenBalancesForAddress, IBalances, ITokenBalance } from 'src/services/covalentService';
...
const BorrowerListItem = ({ lend }: Lends) => {
  ...
  const { data: borrowerAvatar, isError, isLoading } = useEnsAvatar({
      ...
      <HStack flex={1}>
        <Button variant={'ghost'} width='full' justifyContent={'start'}>
          <HStack gap={2}>
            ...
            <Text>{lend.amount} {lend.symbol}</Text>
          </HStack>
        </Button>
        {busy ? <Spinner />
          : <IconButton onClick={onDelete} as={HiOutlineX} size='sm' color={'red.400'} variant='ghost' aria-label='Remove borrower' />
        }
      </HStack>
      </Link>
  )
...
const DepositProfile = (): JSX.Element => {
  ...
  useAsyncEffect(async () => {
    const balancesForUser = await getTokenBalancesForAddress(1, address as string)
  ...
  const borrowers = useMemo(() => {
    return [{
      ...
      symbol: "USDC",
      amount: 123,
      txLink: "https://etherscan.io/tx/0x39eef0326cb3c86c17f3b537514fa557ce183486ad2d9d6c4b40c8a4c0c6798f",
    }]
  }, [address])
            ...
            <Center flexDir={'column'} w='full' gap={5}>
              ...
              <Container>
                <List spacing={3} flex='1'>
                  <ListItem>
                    <Text fontSize={'lg'}>
                      <Icon as={RiUserSharedLine} />&nbsp;Borrowers
                    </Text>
                  </ListItem>
