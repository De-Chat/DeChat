...
import { Tooltip, Text, useDisclosure, Center, VStack, Icon, Spinner } from '@chakra-ui/react';
                ...
                <div className="upload__image-wrapper">
                  {!imageList.length ?
                    <Text
                      ...
                      {...dragProps}
                      {...(isDragging ? { bgGradient: 'linear(to-l, #7928CA, #FF0080)', bgClip: 'text' } : null)}
                    >
                  ...
                  {imageList.map((image, index) => (
                    <Tooltip key={index} hasArrow placement='top' label='Click to change'>
                      <img src={image['data_url']} alt="" height="12rem" onClick={() => onImageUpdate(index)} />
                    </Tooltip>
                  ))}
                </div>
              )}
