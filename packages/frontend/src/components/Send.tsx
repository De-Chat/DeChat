...
const Send = () => {
  ...
  const sendEth = async (address: string) => {
    if (!contract) {
  ...
  const sendERC20 = async (tokenAddr: string, recipient: string, amount: string) => {
    if (!contract) {
      console.warn('Contract not constructed!');
      return;
    }
    try {
      const actualAmount = ethers.utils.parseUnits(amount, 'ether');
      let tsx = await contract.sendEth(tokenAddr);
      const receipt = await tsx.wait();
      console.log({ receipt });
    } catch (e: any) {
      console.error(e);
